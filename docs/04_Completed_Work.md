...
### 세션 5: C-S-H 젤 형성 분석 (2026-01-28)

**소요 시간**: ~3.5시간 (10 ps MD 포함)
**초점**: 장시간 MD 시뮬레이션, Ca 용출, Si 배위, C-S-H 전구체 형성
...
**결과**:
- Ca 용출: 3개 (변화 없음)
- Si 배위수: 4.00 (100% tetrahedral)
- Ca-Si 쌍 < 3.5 Å: 9 → 10 (증가)
...
**판정 기준**: 초기 Ca 중심에서 5.0 Å 이상 이동

**결과**:
- 초기 용출 속도: 1.5 Ca/ps (2 ps 기준)
- 최종 용출 속도: 0.4 Ca/ps (10 ps 기준)
- 시간에 따라 감소 (평형 접근)

#### 작업 4: Si 배위 환경 분석 ✅

**결과**:
- 모든 Si 원자: CN = 4 (100% SiO₄ 사면체)
- 10 ps 동안 변화 없음
- Si-O 결합의 안정성 확인
...
**결과**:
- 최소 Ca-Si 거리: ~3.0 Å (C-S-H 특성 범위 내)
- Ca-Si 쌍 변화 관찰 (구조 재배열)

**생성된 파일**:
- `trajectories/csh_formation_2.0ps.traj`
- `trajectories/csh_formation_10.0ps.traj`
- `figures/csh_formation_overview_2.0ps.png`
- `figures/csh_formation_overview_10.0ps.png`
- `figures/paper/Fig3_CSH_formation.png`
- `results/ca_leaching_*.csv`
- `results/si_coordination_*.csv`
- `results/ca_si_distances_*.csv`
- `results/csh_formation_summary_*.json`
...
| **총 계산 시간** | ~6시간 |
| **GPU 활용** | ~4시간 활성 |
| **절약된 CPU 시간** (비GPU 대비) | ~60-100시간 |
| **최적화된 구조** | 1 (C3S) |
| **MD 시뮬레이션** | 3 (1 ps, 2 ps, 10 ps) |
| **궤적 프레임** | 3,000+ |
